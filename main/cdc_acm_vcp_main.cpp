...
struct UsbDevice {
    ...
    esp_err_t send_data(const uint8_t* data, size_t len) {
        if (!need_close) return ESP_FAIL;
        
        esp_err_t result = ESP_FAIL;
        
        if (is_vcp_device && vcp_device != nullptr) {
            result = vcp_device->tx_blocking((uint8_t*)data, len, 100);
        } else if (!is_vcp_device && cdc_handle != nullptr) {
            result = cdc_acm_host_data_tx_blocking(cdc_handle, (uint8_t*)data, len, 100);
        }
...
static bool vcp_rx_callback(const uint8_t *data, size_t data_len, void *arg) {
    ...
    xSemaphoreTake(tcp_mutex, pdMS_TO_TICKS(10));
    if (active_client_socket >= 0) {
        int sent = 0;
        size_t total_sent = 0;
        
        while (total_sent < data_len) {
            sent = send(active_client_socket, data + total_sent, 
                       data_len - total_sent, 0);
            
            if (sent <= 0) {
                ESP_LOGW(TAG, "Failed to send USB data to TCP");
                break;
            }
            total_sent += sent;
        }
        ...
        if (total_sent > 0) {
            ESP_LOGD(TAG, "Sent %d bytes from USB to TCP", total_sent);
        }
    }
...
static void setup_usb_baudrate(uint32_t baudrate = 115200) {
    if (!usb_device.need_close) return;
...
extern "C" void app_main(void) {
    ...
    xTaskCreate(tcp_server_task, "tcp_server", 8192, NULL, 5, &tcp_server_task_handle);
    
    ESP_LOGI(TAG, "Gateway ready! Connect to:");
